...
public class JettyStatisticsMetrics implements MeterBinder {
    ...
    public void bindTo(MeterRegistry reg) {
        ...
        bindCounter(reg,"jetty.responses.bytes", "Total number of bytes across all responses", StatisticsHandler::getResponsesBytesTotal);
        ...
        bindTimeGauge(reg,"jetty.request.time.max.seconds", "Maximum time spent handling requests", StatisticsHandler::getRequestTimeMax);
        bindTimeGauge(reg,"jetty.stats.seconds", "Time in seconds stats have been collected for", StatisticsHandler::getStatsOnMs);
